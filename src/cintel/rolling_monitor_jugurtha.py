...
Paths (relative to repo root)
    ...
    INPUT FILE: data/system_metrics_timeseries_jugurtha.csv
    OUTPUT FILE: artifacts/rolling_metrics_jugurtha.csv

Terminal command to run this file from the root project folder

    uv run python -m cintel.rolling_monitor_jugurtha
...
DATA_FILE: Final[Path] = DATA_DIR / "system_metrics_timeseries_jugurtha.csv"
OUTPUT_FILE: Final[Path] = ARTIFACTS_DIR / "rolling_metrics_jugurtha.csv"
...
def main() -> None:
    ...
    # ----------------------------------------------------
    # STEP 2.1: DERIVE ERROR RATE COLUMN
    # ----------------------------------------------------

    df = df.with_columns(
        pl.when(pl.col("requests") > 0)
        .then(pl.col("errors") / pl.col("requests"))
        .otherwise(None)
        .alias("error_rate")
    )
    LOG.info("Derived per-observation error_rate column (errors / requests)")
    ...
    # ----------------------------------------------------
    # STEP 3.4: ROLLING MEAN OF ERROR RATE
    # ----------------------------------------------------
    ...
    df_with_rolling = df.with_columns(
        [
            ...
            error_rate_rolling_mean_recipe,
        ]
    )

    LOG.info("Computed rolling mean signals (including error rate)")
